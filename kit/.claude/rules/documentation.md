...
IDs, enums e mensagens de commit seguem o artefato. Arquivos de doc em
...
`plan.md`, `tasks.md`. Relatório sem template degenera num pântano de formatos
(lição de origem: 43 formatos distintos numa única pasta de docs). Índices são
...
resolvido ANTES do portão do refinamento. Doc, spec e prompt novos usam o termo
do glossário. Termo que falta lá é sinal: linguagem inventada (reconsiderar) ou
...
`.claude/templates/adr.md`). Faltando qualquer um dos três, não é ADR. A regra
diz "não reporte"; o ADR diz "por que se escolheu assim" — quando coincidem, a
regra aponta para o ADR.

## O que é gerado e não se edita à mão

| Arquivo | Gerado por |
|---|---|
| {{ARTEFATO_GERADO_1}} | {{BUILDER}} |

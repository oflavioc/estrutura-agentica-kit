...
> Lição de origem: o processo do projeto de origem degradou DUAS vezes — fases
> implementadas sem spec e selagens squashed inauditáveis. Estes gates existem
> por isso.

- **Abertura de fase/marco de projeto**: implementação bloqueada enquanto não
  existir a spec do marco (`specs/{{MARCO}}*.md`) com hash registrado. O rigor
  do primeiro marco é o piso, não o teto.
- **Selagem**: commit de selagem é doc-only, separado da candidata; proibido
  squash em branch de fase; a selagem é a única rota develop→main (tag anotada,
  sob pedido).
- **Auditoria independente humana**: exigida em selagem de fase, mudança de
  superfície congelada pela porta pesada, e empate PO×QA. Demanda comum encerra
  com QA + PO. Nenhum agente escreve em registro de aceitação: agente
  **reprova ou declara "não encontrei objeção"**.
